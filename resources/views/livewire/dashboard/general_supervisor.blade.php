<div>
    @if(request()->query('as_agent'))
        @php
            // Map supervisor data to agent dashboard variables
            $branchSafes = $branchSafes ?? ($branchSafes ?? []);
            $agentTodayTransactionsCount = $agentTodayTransactionsCount ?? 0;
        @endphp
        @include('livewire.dashboard.agent', array_merge(get_defined_vars(), ['user' => $user, 'branchSafes' => $branchSafes, 'agentTodayTransactionsCount' => $agentTodayTransactionsCount]))
        <div class="mt-4 text-right">
            <a href="{{ route('dashboard') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 active:bg-blue-800 transition">Switch to Supervisor Dashboard</a>
        </div>
    @else
        <div class="mb-4 text-right">
            <a href="{{ route('dashboard', array_merge(request()->all(), ['as_agent' => 1])) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring focus:ring-blue-200 active:bg-blue-800 transition">Switch to Agent Dashboard</a>
        </div>
