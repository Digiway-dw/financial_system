                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="safeId" wire:model="safeId" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                                    @foreach ($safes as $safe)
                                        <option value="{{ $safe->id }}">{{ $safe->name }} - Balance: {{ number_format($safe->balance, 2) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            @if($withdrawalType === 'client_wallet')
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="clientCode" id="clientCode" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Enter client code">
                                </div>
                                
                                <!-- Client Mobile -->
                                <div>
                                    <label for="clientNumber" class="block text-sm font-medium text-gray-700 mb-1">Client Mobile</label>
                                    <input type="text" wire:model="clientNumber" id="clientNumber" class="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Enter client mobile number">
                                </div>
                            @endif
