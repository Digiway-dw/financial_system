                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                        <button wire:click="$set('depositType', 'direct')"
                            ...
                            <div class="p-4 text-center">
                                <div class="mx-auto w-10 h-10 rounded-lg flex items-center justify-center mb-2 {{ $depositType === 'direct' ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200' }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                                    </svg>
                                </div>
                                <span class="text-sm font-medium {{ $depositType === 'direct' ? 'text-blue-700' : 'text-slate-700' }}">Direct</span>
                            </div>
                        </button>
                        <button wire:click="$set('depositType', 'client_wallet')"
                            ...
                            <div class="p-4 text-center">
                                <div class="mx-auto w-10 h-10 rounded-lg flex items-center justify-center mb-2 {{ $depositType === 'client_wallet' ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200' }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                </div>
                                <span class="text-sm font-medium {{ $depositType === 'client_wallet' ? 'text-emerald-700' : 'text-slate-700' }}">Client Wallet</span>
                            </div>
                        </button>
                        <button wire:click="$set('depositType', 'user')"
                            ...
                            <div class="p-4 text-center">
                                <div class="mx-auto w-10 h-10 rounded-lg flex items-center justify-center mb-2 {{ $depositType === 'user' ? 'bg-purple-500 text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200' }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
                                    </svg>
                                </div>
                                <span class="text-sm font-medium {{ $depositType === 'user' ? 'text-purple-700' : 'text-slate-700' }}">User</span>
                            </div>
                        </button>
                        <button wire:click="$set('depositType', 'admin')"
                            ...
                            <div class="p-4 text-center">
                                <div class="mx-auto w-10 h-10 rounded-lg flex items-center justify-center mb-2 {{ $depositType === 'admin' ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 group-hover:bg-slate-200' }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                    </svg>
                                </div>
                                <span class="text-sm font-medium {{ $depositType === 'admin' ? 'text-orange-700' : 'text-slate-700' }}">Admin</span>
                            </div>
                        </button>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="submitDeposit" class="space-y-6">
                    ...
                    <div class="relative mb-4">
                        ...
                        <div class="relative">
                            <input type="text" 
                                   ...
                                   placeholder="Enter name, mobile, or customer code" 
                                   autocomplete="off" />
                            @if ($clientId)
                                <button type="button" 
                        ...
                        @endif
                        @error('clientSearch') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" 
                               wire:model.defer="depositorNationalId" 
                               minlength="14" 
                               maxlength="14" 
                               pattern="[0-9]{14}" 
                               class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all" 
                               ...
                               required />
                        @error('depositorNationalId') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-gray-700 font-medium mb-2">Depositor Mobile Number</label>
                        <input type="text" 
                               wire:model.defer="depositorMobileNumber" 
                               minlength="11" 
                               maxlength="15" 
                               pattern="[0-9]+" 
                               class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all" 
                               ...
                               required />
                        @error('depositorMobileNumber') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select wire:model="safeId" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
                            @foreach ($branchSafes as $safe)
                ...
                <button type="submit" 
                        class="w-full bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
