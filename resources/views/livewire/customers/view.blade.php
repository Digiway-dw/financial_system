        ...
        <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-center space-x-4">
                    <div class="w-16 h-16 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl flex items-center justify-center text-white font-bold text-xl">
            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-slate-800">{{ $customer->created_at ? $customer->created_at->format('M d, Y') : 'N/A' }}</p>
                    </div>
            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                <div class="space-y-4">
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
                        ...
                        <div class="text-2xl font-bold text-green-800">{{ number_format($customer->balance, 2) }} EGP</div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-200">
                        ...
                        <div class="text-lg font-bold text-purple-800">{{ number_format($totalTransferred, 2) }} EGP</div>
                    </div>
                    <div class="bg-gradient-to-r from-yellow-50 to-orange-50 p-4 rounded-xl border border-yellow-200">
                        ...
                        <div class="text-lg font-bold text-yellow-800">{{ number_format($totalCommission, 2) }} EGP</div>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ 
                                        $transaction->transaction_type === 'send' ? 'bg-red-100 text-red-700' : 
                                        ($transaction->transaction_type === 'receive' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700') 
                                    }}">
                                        {{ ucfirst($transaction->transaction_type) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm font-mono text-slate-900">{{ number_format($transaction->amount, 2) }} EGP</td>
                                <td class="px-6 py-4 text-sm font-mono text-slate-700">{{ number_format($transaction->commission, 2) }} EGP</td>
                                <td class="px-6 py-4 text-sm text-slate-700">{{ $transaction->line_mobile_number ?? '-' }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ 
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-slate-500">This customer hasn't made any transactions yet.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
