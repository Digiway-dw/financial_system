<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-lg font-semibold text-blue-600">{{ number_format($customerDetails['safe_balance'], 2) }} EGP</p>
                    </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="space-y-3">
                        @foreach($branchDetails['safe_balances'] as $safe)
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                ...
                                <span class="text-lg font-semibold text-green-600">{{ number_format($safe['balance'], 2) }} EGP</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        @foreach($branchDetails['line_balances'] as $line)
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                ...
                                <span class="text-lg font-semibold text-blue-600">{{ number_format($line['balance'], 2) }} EGP</span>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        {{-- Loading States --}}
        <div wire:loading.block class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-xl p-6 flex items-center space-x-4 rtl:space-x-reverse">
                <svg class="animate-spin h-6 w-6 text-blue-600" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <span class="text-lg font-medium text-gray-900">جاري تحميل التقرير...</span>
            </div>
        </div>
    </div>
</div>
