<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-10">
    <div class="max-w-xl mx-auto bg-white rounded-2xl shadow-xl p-8">
        <h3 class="text-2xl font-bold text-gray-900 mb-6">Edit Line</h3>
        <form wire:submit.prevent="updateLine" class="space-y-6">
            ...
            <div>
                ...
                <x-text-input wire:model="currentBalance" id="currentBalance" name="currentBalance" type="number"
                    step="1" min="0" class="mt-1 block w-full" required
                    oninput="this.value = this.value.replace(/[^\d]/g, '');" />
                <x-input-error class="mt-2" :messages="$errors->get('currentBalance')" />
                <p class="text-xs text-gray-500 mt-1">Whole numbers only (no decimals allowed)</p>
            </div>

            <!-- Daily Limit -->
            <div>
                <x-input-label for="dailyLimit" :value="__('Daily Limit (EGP)')" />
                <x-text-input wire:model="dailyLimit" id="dailyLimit" name="dailyLimit" type="number" step="1"
            ...
            <div class="flex items-center gap-4 mt-8">
                <x-primary-button class="px-8 py-3 text-base">{{ __('Update Line') }}</x-primary-button>
