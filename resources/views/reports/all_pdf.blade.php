...
<body>
    ...
    <div class="section">
        ...
        <table class="summary-table">
            ...
            <tr>
                <th>العمولة النهائي</th>
                <td>{{ number_format(($financialSummary['commission_earned'] ?? 0) - ($financialSummary['total_discounts'] ?? 0), 2) }}
                    EGP</td>
            </tr>
            <tr>
                <th>الخصم</th>
                <td>{{ number_format($financialSummary['total_discounts'] ?? 0, 2) }} EGP</td>
            </tr>
            <tr>
                <th>الربح الصافي</th>
                <td>{{ number_format($financialSummary['net_profit'] ?? 0, 2) }} EGP</td>
            </tr>
        </table>
    </div>
    <div class="section">
            ...
            <tbody>
                @foreach ($customerBalances as $customer)
                    <tr>
                        ...
                        <td>{{ number_format($customer['balance'], 2) }} EGP</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
            ...
            <tbody>
                @foreach ($transactions as $transaction)
                    <tr>
                        ...
                        <td>{{ number_format(($transaction['commission'] ?? 0) - ($transaction['deduction'] ?? 0), 2) }}
                            EGP</td>
                        ...
                        <td>{{ \Carbon\Carbon::parse($transaction['transaction_date_time'])->format('d/m/y h:i A') }}
                        </td>
                    </tr>
                @endforeach
            </tbody>
